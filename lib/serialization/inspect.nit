...
# Simple serializer reading object attributes
private class InspectSerializer
	...
	private var first_object: nullable Object = null
	...
	do
		...
		if cache.has_object(object) then
			...
			var id = cache.new_id_for(object)
			stream.write "<"
			stream.write object.class_name
			stream.write ":#"
			stream.write id.to_s
...
redef class Serializable
	...
	# Improve the default inspection recursively reading serializable attributes
	#
	...
	# ~~~
	#
	# Inspections of mutable object show their dynamic type and an id unique
	# to each call to `inspect`. Items of collections are flattened:
	#
	# ~~~
	...
	# assert class_with_null.to_s ==
	#        "<MyClass:#0, i: 123, o: null>"
	#
	# var class_with_other = new MyClass(456, class_with_null)
	# assert class_with_other.to_s ==
	#        "<MyClass:#0, i: 456, o: <MyClass:#1>>"
	# ~~~
	#
	# Cycles in an inspection are identified with a special `<ref` block:
	#
	# ~~~
	#
	# var class_with_cycle = new MyClass(789)
	# class_with_cycle.o = class_with_cycle
	# assert class_with_cycle.to_s ==
	#        "<MyClass:#0, i: 789, o: <ref MyClass:#0>>"
	# ~~~
	#
	...
	# assert long_class.to_s == "<MyClass:#0, i: 123456789, o: \"Some very very very very very very very very lon…"
	# ~~~
	redef fun inspect
	do
