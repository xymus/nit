...
class SeparateCompilerVisitor
		...
		if value.mtype == mtype then
			...
			if mtype.is_tagged then
				if mtype.name == "Int" then
					return self.new_expr("(int64_t)({value})>>2", mtype)
				else if mtype.name == "Char" then
					return self.new_expr("(uint32_t)((int64_t)({value})>>2)", mtype)
				else if mtype.name == "Bool" then
					return self.new_expr("(short int)((int64_t)({value})>>2)", mtype)
				else
					abort
				end
			end
			...
			if value.mtype.is_tagged then
				...
				if value.mtype.name == "Int" then
					...
					res = self.new_expr("(val*)((int64_t)({value})<<2|2)", mtype)
				else if value.mtype.name == "Bool" then
					res = self.new_expr("(val*)((int64_t)({value})<<2|3)", mtype)
				else
					abort
				end
	...
	do
		...
		return "((int64_t){value}&3)" # Get the two low bits
	end
