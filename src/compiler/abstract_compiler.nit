	...
	do
		...
		if cname == "Int" then
			...
			if pname == "object_id" then
				v.ret(v.new_expr("(int64_t){arguments.first}", ret.as(not null)))
				return true
				...
				v.ret(v.new_expr("(int64_t){arguments[0]}", ret.as(not null)))
				return true
			end
			...
			if pname == "[]" then
				...
				v.ret(v.new_expr("(int64_t)*((uint32_t*)({arguments[0]} + {arguments[1]}))", ret.as(not null)))
				return true
			else if pname == "fetch_4_hchars" then
				v.ret(v.new_expr("(int64_t)be32toh(*((uint32_t*)({arguments[0]} + {arguments[1]})))", ret.as(not null)))
				return true
			end
