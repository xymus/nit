...
private class ExplainAssertPhase
	...
	do
		...
		var visitor = new UsefulAssertVisitor(toolcontext, mmodule, string_class.mclass_type)
		visitor.enter_visit nmodule
	end
end

# Visitor to find and explain asserts
private class UsefulAssertVisitor
	super Visitor
	...
	#
	# Add it to `explain_str` if `autoadd == true`, the default.
	fun explain_string(value: String, autoadd: nullable Bool): AStringExpr
	do
		autoadd = autoadd or else true
		...
		if autoadd then explain_str.n_exprs.add op
		return op
	end

	# Add the value of `v_expr` to `explain_str` and protect null values
	fun explain_expr(v_expr: AVarExpr)
	do
		var variable = v_expr.variable
		var mtype = v_expr.mtype

		if variable == null or mtype == null then
			# There was an error in a previous phase
			explain_string("<unexpected expression>")
			return
		end

		var expr = builder.make_var_read(variable, mtype)
...
redef class AExpr
	...
	private fun accept_explain_assert(v: UsefulAssertVisitor)
	do v.explain_expr(make_var_read)
end
...
redef class ABinopExpr
	...
	do
		...
		v.explain_expr n_expr.make_var_read
		v.explain_string(" {n_op.text} ")
		v.explain_expr n_expr2.make_var_read
	end
end
